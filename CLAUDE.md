...
CI (`.github/workflows/ci.yml`) runs `compile`, `format:check`, `test` and both builds on every push and pull request.
...
Unit tests: `npm test` (Vitest, `tests/`). Browser-level checks: the scripts in `probes/` (`npm run probe:toggle`, `probe:hover`, `probe:messaging`, `probe:search-keys`, `probe:origin`, `probe:toggle:firefox`) drive the built extension in headless Edge or Firefox against youtube.com, so they need those browsers and network access and are not part of CI. Beyond those, verify changes by building and loading `.output/chrome-mv3/` (or `.output/firefox-mv2/`) in a browser and watching the console for `[SubPeek]` lines. "ytcfg not found in page scripts" or "fetch PlayerResponseInnerTube error" mean the primary InnerTube path failed and the extension is running on the slower watch-page fallback.
...
- **Content script** (`content/index.ts` -> `content/main.ts`): injected on youtube.com at `document_end` with `cssInjectionMode: "manifest"`. `index.ts` subscribes to `Settings.enabled` (with `init: true`) and calls `start()` / `stop()` from `main.ts`. `main.ts` owns the lifecycle and a single `MutationObserver` on `document.documentElement` that hands every added element to two modules under `content/youtube/`:
    ...
    2. `preview.ts` - `watchPreviewsIn(root)` handles YouTube's inline hover preview (`ytd-video-preview`), one global element positioned over the hovered card from outside its DOM, which would cover the badge; while it is active the badge is mirrored into the preview's player box (`.ytbext-preview-host`) so it stays clickable. `probes/hover-check.mjs` is the regression check.

    `start()` and `stop()` are idempotent mirror images guarded by a `running` flag, and they are the only places the content script touches the page or subscribes to settings; no module does either at import time. `start()` mounts the dev metrics overlay, subscribes `rerenderEmbeds()` to the favorite languages and starts the observer. `stop()` disconnects the observer, drops that subscription, closes the in-page popup, calls `stopPreviews()` and `stopThumbnails()`, which disconnect their own observers, remove preview hosts, unwrap every `.ytbext-thumbnail-wrapper` (`wrapper.replaceWith(img)`) and strip the `data-ytbext-processed` markers, and removes the overlay, so the page is left as if the extension were not installed; a later `start()` re-scans from scratch. `initEmbed()` checks `container.isConnected` after its lookup resolves so a request that was in flight during `stop()` renders nothing. `probes/toggle-check.mjs` (Chrome, via headless Edge) and `probes/toggle-check-firefox.mjs` (headless Firefox over WebDriver BiDi) are the regression checks for this cycle.
...
- Primary path: POST to `${location.origin}/youtubei/v1/player` with the ytcfg context. The URL must be absolute; Firefox content scripts do not resolve relative fetch URLs. Fallback: fetch the watch page, locate `var ytInitialPlayerResponse = {` and extract the object with `balancedObject()` from `ytcfg.ts`. A lazy regex up to the first `};` is not enough: JSON strings do not escape `}`, so a description containing code truncated the capture and the video rendered as unavailable. `tests/api.test.ts` is the regression check (it drives `resolveVideoInfo()` against synthetic InnerTube replies and watch pages with a stubbed `fetch`).
...
- `parseVideoResponse()` marks ASR captions with `isAutoGenerated: true` and the original audio track with `isOriginal: true`; audio tracks are de-duplicated by language code. The original is identified by the `audioTrack.id` suffix (`<lang>.4`; `.3` is a creator dub, `.10` a YouTube auto-dub). Do not use `audioIsDefault` for this: it marks the track YouTube auto-plays for the viewer's UI language, which on a non-English video viewed in English is the English dub. `displayName` is localised, so matching "original" does not work either.
...
- **`common/storage.ts`**: `ReactiveStorage` - reactive wrapper around a `browser.storage` area with an in-memory cache and per-key change listeners fed by `storage.onChanged` (so changes made in the popup propagate to content scripts). Listeners are kept in a plain `Map<string, Set<callback>>`, not an `EventTarget`: a `new EventTarget()` constructed inside a Firefox content script never delivers events to its listeners (`dispatchEvent` returns `true`, nothing runs), which left every subscriber (`start()`/`stop()`, favorite re-render) dead on Firefox while the cache still updated. Real DOM nodes are fine, so the `ytbext:render` event on embed containers is unaffected. `settings.ts` owns the single instance (`storage.local`); consumers go through `Settings` and `await Settings.ready()` once per context before reading
- **`common/settings.ts`**: `Settings.ready()` plus `Settings.langCodes` with `.get()`, `.set()`, `.subscribe()`, `.add()` and `.remove()` (default `["en"]`) and `Settings.enabled` with `.get()`, `.set()`, `.subscribe()` (default `true`). `.subscribe()` returns a function that removes the subscription. Keys are prefixed `SETTINGS:`
- **`common/idb.ts`**: generic Promise-based `IDBStore<T>` (`get`, `put`, `deleteByIndexRange`) for one object store; a version bump drops and recreates the store, and open connections close on `versionchange` so another tab's upgrade is never blocked
- **`common/cache.ts`**: `VideoCache` singleton built on `IDBStore`, keyed by `videoId` with a `timestamp` index. `get()` returns only entries younger than `CACHE_TTL_SECONDS` (defined there), so the content script never sees a stale one, and `cleanExpired()` sweeps the rest at startup. Used only by the background script
- **`content/constants.ts`**: BEM class names (`CSS`; build no class names outside it, so grep finds every use) and custom DOM event names (`EVENT`)
...
- **`content/debugging.ts`**: dev-only (`import.meta.env.DEV`) metrics overlay (fetch counts, cache hits, rate limits), mounted by `start()` and removed by `stop()` (`mountMetricsOverlay()` is a no-op in production builds). `metricsProxy` is a `Proxy` that re-renders on every write; production code increments it freely and it is a no-op when the overlay is not mounted
...
Prettier with 4-space indent and `prettier-plugin-organize-imports` (`.prettierrc.json`). `npm run format` writes, `npm run format:check` verifies; CI runs the check. Line endings are LF everywhere (`.gitattributes`).
...
- Cache TTL is a fixed 30 minutes (`CACHE_TTL_SECONDS` in `common/cache.ts`, enforced only there); cache timestamps are in seconds (`Date.now() / 1000`), not milliseconds
- Settings are reactive: components subscribe and re-render, they never poll storage
- Content-script modules have no side effects at import: anything that touches the page or subscribes to settings is wired in `start()` and undone in `stop()` (`content/main.ts`)
