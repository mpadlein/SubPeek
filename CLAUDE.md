...
- **`common/settings.ts`**: `Settings.ready()` plus `Settings.langCodes` with `.get()`, `.set()`, `.subscribe()`, `.add()` and `.remove()` (default `["en"]`) and `Settings.enabled` with `.get()`, `.set()`, `.subscribe()` (default `true`). Keys are prefixed `SETTINGS:`
